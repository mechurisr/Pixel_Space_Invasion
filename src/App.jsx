...
function App() {
  ...
  const handleSelect = (country) => {
    if (gameState === 'SELECT_START') {
      ...
      const aiInitData = AI_FACTIONS.map((faction, index) => ({
        ...
        territoryIds: [shuffled[index].id]
      }))
  ...
  const handlePlayerInvasion = (target) => {
    ...
    if (pCountry.military > target.military) {
      // Win
      setTerritories(prev => prev.map(t => {
        if (t.id === target.id) return { ...t, military: Math.floor(t.military / 2), isOccupied: false, mutationUnit: null }
        if (t.id === pCountry.id) return { ...t, military: Math.max(0, t.military - Math.floor(target.military / 2)) }
        return t
      }))
      ...
      // Lose
      setTerritories(prev => prev.map(t => t.id === pCountry.id ? { ...t, military: Math.max(0, t.military - 30) } : t))
  ...
  const handleNextTurn = () => {
    ...
    newAiData.forEach(faction => {
      ...
      currentTerritoryIds.forEach(tId => {
        ...
        if (validNeighbors.length > 0 && t.military > 40) {
          ...
          const vulnerableNeighbors = validNeighbors
            ...
            .filter(n => t.military > (n.military + 10))
        ...
        if (targetAcquired) {
          ...
          newTerritories[tIndex].military = Math.max(0, t.military - Math.floor(target.military / 2))
    ...
    // Random Alien Invasion Event (Turn >= 2) - Increased chance to 85%
    if (turn >= 2) {
      ...
      if (unOccupied.length > 0 && Math.random() > 0.15) { // 85% chance to spawn aliens
        const target = unOccupied[Math.floor(Math.random() * unOccupied.length)]
        if (target.military < 70) {
          ...
          newTerritories[tIdx] = { ...newTerritories[tIdx], isOccupied: true, mutationUnit: mut }
          updateLogs.push(`${target.name}: OCCUPIED BY ALIENS. ${mut} SPAWNED.`)
        }
      }
    }
